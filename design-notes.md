...
- **Correction (2026-06-10 sync):** the speaker **declined** the "call what's dead" rewrite and the standalone `NOT MINE TO GRANT` label below. Deck stands: need-band item 2 = **"rank which views matter, allow UX changes"**, qualifier = **inline green "& not mine to grant"** on the "What I'd need" header. Net effect: S24 loses the backward-looking "call what's dead" vacuum beat — the need band is forward-only ("allow UX changes"). The only new thing added to S24 is the two-beat build. The reasoning below is kept as history.
